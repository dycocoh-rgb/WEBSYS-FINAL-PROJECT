...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    if (strlen($name) < 3) {
        ...
        $duplicate = $conn->query("SELECT event_id FROM event WHERE event_name='$name' AND event_date='$date'")->fetch_assoc();
        if ($duplicate) {
            $error = "An event with this name already exists on that date.";
        } else {
